 ...
 * single edit away at all times. */
{
  ...
  /* Only declared fields are written back, so an undeclared org key is deleted
   * the next time somebody saves the announcement bar in the CMS. */
  const cmsSrc = fs.readFileSync(ROOT + "/admin/config.yml", "utf8");
  ok(/name: rollCount/.test(cmsSrc), "and it is declared in the CMS, so a banner save cannot drop it");
}
